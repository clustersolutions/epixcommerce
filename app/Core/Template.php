 ...
 */
    ...
    public function parseContent($content, $whitelist = null) {
      ...
      $le = function ($matches) use (&$le, &$pattern, $whitelist) {
        ...
        if ( class_exists('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Module\\Template\\Tag\\' . $matches[1]) ) {
          $tag_class = 'OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Module\\Template\\Tag\\' . $matches[1];
        } elseif ( class_exists('OSC\\Core\\Template\\Tag\\' . $matches[1]) ) {
          $tag_class = 'OSC\\Core\\Template\\Tag\\' . $matches[1];
        }

        if ( isset($tag_class) ) {
          if ( (isset($loaded_tags[$matches[1]]) && ($loaded_tags[$matches[1]] === true) ) || is_subclass_of($tag_class, 'OSC\\Core\\Template\\TagAbstract') ) {
 ...
 */
    ...
    public function valueExists($key, $autoload = true) {
      if (($autoload === true) && array_key_exists($key, $this->_values) === false) {
        if (class_exists('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Module\\Template\\Value\\' . $key . '\\Controller') && is_subclass_of('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Module\\Template\\Value\\' . $key . '\\Controller', 'OSC\\Core\\Template\\ValueAbstract')) {
          call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Module\\Template\\Value\\' . $key . '\\Controller', 'initialize'));
        } elseif (class_exists('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Module\\Template\\Value\\' . $key . '\\Controller') && is_subclass_of('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Module\\Template\\Value\\' . $key . '\\Controller', 'OSC\\Core\\Template\\ValueAbstract')) {
          call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Module\\Template\\Value\\' . $key . '\\Controller', 'initialize'));
        }
      }
