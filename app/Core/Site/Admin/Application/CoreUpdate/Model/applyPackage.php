...
// directories (see http://bugs.php.net/bug.php?id=54289)
      ...
      if ( $phar_can_open === true ) {
        if ( isset($meta['run']) && method_exists('OSC\\Work\\CoreUpdate\\' . $meta['run'] . '\\Controller', 'runAfter') ) {
          $results = call_user_func(array('OSC\\Work\\CoreUpdate\\' . $meta['run'] . '\\Controller', 'runAfter'));
