  ...
  class getAccessModules {
    public static function execute() {
      ...
      foreach ( $DLapps->getFiles() as $file ) {
        if ( !in_array($file['name'], call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Controller', 'getGuestApplications'))) && file_exists($DLapps->getDirectory() . '/' . $file['name'] . '/Controller.php') ) {
      ...
      foreach ( $DLcapps->getFiles() as $file ) {
        if ( !in_array($file['name'], $module_files) && !in_array($file['name'], call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Controller', 'getGuestApplications'))) && file_exists($DLcapps->getDirectory() . '/' . $file['name'] . '/Controller.php') ) {
          $module_files[] = $file['name'];
        }
      }
      ...
      foreach ( $module_files as $module ) {
        $application_class = 'OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . $module . '\\Controller';
