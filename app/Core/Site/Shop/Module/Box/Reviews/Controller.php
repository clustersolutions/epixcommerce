 ...
 */
  ...
  namespace OSC\Core\Site\Shop\Module\Box\Reviews;
  ...
  class Controller extends \OSC\Core\Modules {
    ...
    public function initialize() {
      ...
      if ( $OSCOM_Service->isStarted('Reviews') ) {
        if ( (BOX_REVIEWS_CACHE > 0) && $OSCOM_Cache->read('box-reviews' . (isset($OSCOM_Product) && ($OSCOM_Product instanceof \OSC\Site\Shop\Product) && $OSCOM_Product->isValid() ? '-' . $OSCOM_Product->getID() : '') . '-' . $OSCOM_Language->getCode(), BOX_REVIEWS_CACHE) ) {
          ...
          if ( isset($OSCOM_Product) && ($OSCOM_Product instanceof \OSC\Site\Shop\Product) && $OSCOM_Product->isValid() ) {
