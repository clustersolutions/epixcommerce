...
class Controller implements \OSC\Core\SiteInterface
{
    ...
    public static function initialize()
    {
        ...
        try {
            ...
            ob_start(function($buffer) {
                foreach (headers_list() as $h) {
                    if (stripos($h, 'Location:') !== false) {
                        ...
                        \OSC\Core\HttpRequest::setResponseCode(403); // ob_start() needs full namespace

                        $buffer = json_encode([
                            'rpcStatus' => constant('OSC\\Core\\Site\\RPC\\Controller::STATUS_REDIRECT_DETECTED')
                        ]);
            ...
            call_user_func([
                'OSC\\Core\\Site\\' . $site . '\\Controller',
                'initialize'
            ]);

            ob_end_flush();

            if (!call_user_func(['OSC\\Core\\Site\\' . $site . '\\Controller', 'hasAccess'], $application)) {
            ...
            for ($i = $run_pos, $n = count($_GET); $i < $n; $i++) {
                ...
                if (static::siteApplicationRPCExists(implode('\\', $rpc))) {
                    call_user_func(['OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\RPC\\' . implode('\\', $rpc), 'execute']);
    ...
    public static function siteApplicationRPCExists($rpc, $application = null)
    {
        ...
        return class_exists('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . $application . '\\RPC\\' . $rpc);
    }
}
