 ...
 */
    ...
    public static function getUserLevels($id, $site = null) {
      ...
      if ( in_array('*', $applications) ) {
        ...
        foreach ( $DLapps->getFiles() as $file ) {
          if ( !in_array($file['name'], call_user_func(array('OSC\\Core\\Site\\' . $site . '\\Controller', 'getGuestApplications'))) && file_exists($DLapps->getDirectory() . '/' . $file['name'] . '/Controller.php') ) {
        ...
        foreach ( $DLcapps->getFiles() as $file ) {
          if ( !in_array($file['name'], $applications) && !in_array($file['name'], call_user_func(array('OSC\\Core\\Site\\' . $site . '\\Controller', 'getGuestApplications'))) && file_exists($DLcapps->getDirectory() . '/' . $file['name'] . '/Controller.php') ) {
            $applications[] = $file['name'];
          }
        }
      }
      ...
      foreach ( $applications as $app ) {
        $application_class = 'OSC\\Core\\Site\\' . $site . '\\Application\\' . $app . '\\Controller';
    ...
    public static function hasAccess($site, $application) {
      return in_array($application, call_user_func(array('OSC\\Core\\Site\\' . $site . '\\Controller', 'getGuestApplications'))) || isset($_SESSION[$site]['access'][$application]);
    }
  }
?>
