...
abstract class ApplicationAbstract
{
    ...
    {
        return class_exists('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Action\\' . $action);
    }
    ...
    public function runAction($actions)
    {
        ...
        foreach ($actions as $action) {
            ...
            if ($this->siteApplicationActionExists(implode('\\', $run))) {
                call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Action\\' . implode('\\', $run), 'execute'), $this);
            } else {
                break;
            }
        }
    ...
    public function runActions()
    {
        foreach ($this->getRequestedActions() as $action) {
            ...
            call_user_func(array('OSC\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Action\\' . implode('\\', $this->_actions_run), 'execute'), $this);
        }
    }
