...
class Hash
{
    ...
    public static function get(string $string, string $driver = null): string
    {
        if (!isset($driver)) {
            foreach (static::$_drivers as $d) {
                if (call_user_func(['OSC\\Core\\Hash\\' . $d, 'canUse'])) {
                    $driver = $d;
        ...
        return call_user_func(['OSC\\Core\\Hash\\' . $driver, 'get'], $string);
    }
    ...
    public static function validate(string $plain, string $hashed, string $driver = null): bool
    {
        ...
        return call_user_func(['OSC\\Core\\Hash\\' . $driver, 'validate'], $plain, $hashed);
    }

    public static function getRandomString(int $length, string $type = 'mixed'): string
    {
        if (!in_array($type, ['mixed', 'chars', 'digits'])) {
            trigger_error('OSC\\Core\\Hash::getRandomString() $type not recognized:' . $type, E_USER_ERROR);

            return '';
        }

        if ($length < 1) {
            trigger_error('OSC\\Core\\Hash::getRandomString() $length must be 1 or higher value', E_USER_ERROR);
